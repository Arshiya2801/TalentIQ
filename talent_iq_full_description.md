...
### 5. 🏗️ High-Level Logic (The "Magic")
...
- **Layered Architecture**: Clear separation of routes, controllers, and models.
- **Environment Driven**: Fully configurable via `.env` files for security and portability.
- **Reusable Hooks**: Custom React hooks (`useSessions`, `useStreamClient`) for clean logic sharing.
- **Confetti & Micro-animations**: Visual feedback for successful code submission and state transitions.

---

## 📚 Built-in Problem Library
The platform comes pre-loaded with classic technical interview questions, including:
- **Two Sum**: Array/Hash Table manipulation.
- **Reverse String**: In-place string reversal using two pointers.
- **Valid Palindrome**: String processing and logic.
- **Maximum Subarray**: Dynamic Programming (Kadane's Algorithm).
- **Container With Most Water**: Array optimization using two pointers.

Each problem includes **Starter Code** and **Expected Output** validation for JavaScript, Python, and Java.

---

## 🛠️ Troubleshooting Common UI & Server Issues

### ❌ MongoDB: `ECONNREFUSED`
**Cause**: Network restriction or missing IP whitelist.
**Fix**: Add `0.0.0.0/0` (Allow all) or your specific IP to the **Network Access** tab in MongoDB Atlas.

### ❌ PowerShell: `UnauthorizedAccess`
**Cause**: Windows execution policy blocks `npm` scripts.
**Fix**: Run `Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser` in PowerShell as Admin.

### ❌ Stream/Clerk: `Invalid API Key`
**Cause**: Mismatch between `.env` values or missing `VITE_` prefix on the frontend.
**Fix**: Ensure `frontend/.env` variables start with `VITE_` and match the backend keys exactly.

---

## 🔒 Security & Data Integrity
- **Route Protection**: All sensitive API routes (`/api/sessions`, `/api/chat`) are protected by **Clerk Middleware**, ensuring only authenticated users can create or join interviews.
- **Execution Isolation**: Code is sent to the **Piston API**, which executes the logic in a sandboxed environment, preventing malicious code from affecting the application server.
- **Background Integrity**: **Inngest** ensures that user deletions in Clerk are reflected in the application database and Stream, preventing "ghost" users or orphaned data.
